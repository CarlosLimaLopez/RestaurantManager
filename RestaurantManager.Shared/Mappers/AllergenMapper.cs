namespace RestaurantManager.Allergens
{
    public static class AllergenMapper
    {
        ...
        public static IEnumerable<AllergenGetResponse> GetAllAsResponse()
            => GetAll().Select(ToResponse).ToArray();

        public static AllergenGetResponse ToResponse(this AllergenType allergenType)
        {
            var allergen = GetByEnum(allergenType);
            if (allergen == null)
            {
                return new AllergenGetResponse
                {
        ...
        public static AllergenGetResponse ToResponse(Allergen allergen)
        => new()
        {
            Type = allergen.Type.ToString(),
            Name = allergen.Name,
